...
export default function CartScreen() {
    ...
    if (items.length > 0) {
      abandonmentTimer.current = setTimeout(async () => {
        try {
          ...
          if (!domain) return;
          await fetch(`https://${domain}/api/notifications`, { method: 'POST', headers: { 'Content-Type': 'application/json' } });
        } catch {}
      }, ABANDONMENT_MINUTES * 60 * 1000);
    }
    return () => { if (abandonmentTimer.current) clearTimeout(abandonmentTimer.current); };
  }, [items.length]);
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const res = await fetch(`https://${domain}/api/checkout/session`, {
        ...
        body: JSON.stringify({ lineItems, successUrl: `https://${domain}`, cancelUrl: `https://${domain}` }),
      });
  ...
  const renderItem = ({ item }: { item: CartItem }) => {
    const atMax = item.quantity >= MAX_QTY_PER_ITEM;
    return (
      <View style={[styles.item, { backgroundColor: colors.card }]}>
        ...
        <View style={styles.itemInfo}>
          ...
          <View style={styles.qtyControls}>
            {/* Only show minus when qty > 0 — removing entirely when it hits 0 */}
            <TouchableOpacity
            ...
            {/* + button is hidden when at max (1) — anti-hoarding */}
            {!atMax ? (
              <TouchableOpacity
                style={[styles.qtyBtn, { borderColor: colors.border }]}
                onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); updateQuantity(item.priceId, item.quantity + 1); }}
              ...
              <View style={[styles.qtyBtn, { borderColor: 'transparent', backgroundColor: 'transparent' }]}>
                <Text style={{ fontSize: 10, color: colors.mutedForeground }}>max</Text>
              </View>
            )}
          </View>
        </View>
      ...
      {items.length === 0 ? (
        ...
        <>
          <FlatList
            ...
            contentContainerStyle={{ padding: 16, paddingBottom: 240, gap: 10 }}
...
const styles = StyleSheet.create({
  ...
  item: { flexDirection: 'row', alignItems: 'center', borderRadius: 16, padding: 12, gap: 12, shadowColor: '#000', shadowOpacity: 0.04, shadowRadius: 6, shadowOffset: { width: 0, height: 1 }, elevation: 1 },
  ...
  footer: { position: 'absolute', bottom: 0, left: 0, right: 0, borderTopWidth: 1, padding: 20, gap: 10 },
  ...
  checkoutBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, paddingVertical: 15, borderRadius: 14, marginTop: 4 },
